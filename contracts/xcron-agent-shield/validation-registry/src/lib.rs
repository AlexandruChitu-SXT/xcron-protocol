    ...
    + config::ConfigModule
{
    ...
    fn validation_request(
        ...
        // Authorization Check: Restrict validator nomination.
        // The Agent must select a validator that is registered in the whitelist.
        // Check local Oracle whitelist or fail:
        require!(self.whitelisted_validators().contains(&validator_address), "Unauthorized Validator Node");
    ...
    fn validation_response(
        ...
        // State Transition Validation: Enforce status progression based on score
        // Only valid scores above threshold allow progression to Verified state.
        let job_mapper = self.job_data(&updated_data.job_id);
        if !job_mapper.is_empty() {
            job_mapper.update(|job| {
                if updated_data.response >= 70 {
