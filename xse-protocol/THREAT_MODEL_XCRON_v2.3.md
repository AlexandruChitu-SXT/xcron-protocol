# XCron Protocol Threat Model & Security Framework (v2.3 - Enterprise Resilient)

This document is the definitive Threat Model and Security Framework for XCron Protocol (v2.3), incorporating formal verification of ZK circuits, cryptographic binding of hardware attestation, verifiable delay functions (VDF), and strict mitigation of hardware-level side-channel attacks.
...
| **T-15** | Assignment predictability via block seed `get_block_random_seed()`. | Cryptographic | High | transparent Class Group VDF sequential computation (300ms sequential delay). | `contracts/scheduler/src/vdf_assignment.rs` |
| **T-16** | Prover Circuit / zkVM compiler bugs in ZK-PQ verification. | Software | High | Formal verification of circuits (e.g. Picus), public input binding, and on-chain fallback. | ZK-PQ execution pipeline |
| **T-17** | Host-to-Enclave L3 cache and memory side-channel timing leaks on EC2. | Physical | High | NUMA Node Pinning + Disable Hyper-Threading + Constant-Time + vsock Padding. | Enclave configuration |
...
| **T-19** | Replay/Relay of old TEE Attestations. | Protocol | High | Statement ZK proof explicitly binds the unique `TaskHash` or Task nonce. | `contracts/scheduler/src/execution.rs#L111` |

---

## 🔒 Detailed Threat Profiles & Mitigations

### 1. Application & Smart Contract Safety (T-14, T-19)
*   **Threat (T-14):** A malicious target contract re-enters the Scheduler during the cross-shard callback loop of Supernova (which takes 5 to 10 blocks) to drain the user's gas escrow.
*   **Mitigation:** 
...
*   **Threat (T-19):** An attacker intercepts a valid TEE attestation document and ZK-proof from a prior task execution and relays them to execute a different task or double-claim a reward.
*   **Mitigation:**
    1.  **Task Binding in Public Input:** The public inputs of the ZK-proof must explicitly include the unique `TaskHash` or the task's nonce.
    2.  **L1 Verification:** The L1 contract verifies that the ZK-proof's public inputs match the exact task currently being settled in `settle_xse_task`. Any attempt to reuse a proof for a different task hash results in a verification failure.
...
*   **Threat (T-16):** Bugs in the zkVM (SP1/Risc0) or the Dilithium verification circuit (e.g., underconstrained polynomial calculations) allow generation of false proofs.
*   **Mitigation:**
...
*   **Mitigation:**
    ...
    2.  **Disable Hyper-Threading:** Hyper-threading is disabled on the enclave cores, ensuring that no host process can run on the logical thread of the same physical CPU core.
    3.  **Constant-Time Enclave Logic:** All computations inside the enclave (not just the L1 precompile) are written in constant-time using Rust's `subtle` crate, eliminating timing-based branches.
    4.  **Time Padding:** Enclave responses over `vsock` are padded with a strict timer delay, releasing the output at a fixed interval (e.g., exactly 300ms) to prevent execution latency leaks.
    5.  **Memory Blinding:** A background thread writes pseudo-random data to dummy memory blocks to inject noise on the memory bus, flat-lining DRAM bus analysis profiles.
    6.  **Attestation Binding:** The ZK-proof generated off-chain binds the TEE attestation by inserting the Enclave's PCR0 measurement hash and ephemeral public key $pk_{enc}$ directly into the public inputs of the SNARK. The L1 verifier rejects any ZK-proof not signed by a valid TEE attestation certificate.
